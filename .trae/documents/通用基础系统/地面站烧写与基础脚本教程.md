...
PX4 在启动时会优先检查 TF 卡上的 `/fs/microsd/etc/rc.txt`（入口脚本 `rcS`：`ROMFS/px4fmu_common/init.d/rcS:124-131`）。若存在该文件，则执行你自定义的流程，默认自动机型/控制/导航不再加载。
...
* 入口脚本与覆盖：`ROMFS/px4fmu_common/init.d/rcS:124-131, 231-244, 108-114`

* 传感器启动：`rcS:322-366` 与 `ROMFS/px4fmu_common/init.d/rc.sensors`

* 估计器选择：`rcS:369-385`

* MAVLink 与串口：`rcS:504-523`；串口生成 `Tools/serial/generate_config.py`

* RTCM 注入链：`src/modules/mavlink/mavlink_receiver.cpp:2603-2620` → 驱动 `src/drivers/gps/devices/src/gps_helper.h:288-297`
